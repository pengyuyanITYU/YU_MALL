...
public class BeanUtils extends BeanUtil {
     ...
     * @return 目标对象
     */
    public static <R, T> T copyBean(R source, Class<T> clazz, Convert<R, T> converter) {
    ...
    public static <R, T> List<T> copyList(List<R> list, Class<T> clazz, Convert<R, T> converter) {
        if (list == null || list.size() == 0) {
            return CollUtils.emptyList();
        }
        return list.stream().map(r -> copyBean(r, clazz, converter)).collect(Collectors.toList());
    }
}